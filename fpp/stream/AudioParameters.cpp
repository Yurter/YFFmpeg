...
namespace fpp {
    ...
    void AudioParameters::completeFrom(const SharedParameters other) {
        ...
        const auto other_audio_parames = std::static_pointer_cast<AudioParameters>(other);
    ...
    bool AudioParameters::betterThen(const SharedParameters& other) {
        const auto other_aparams = std::static_pointer_cast<const AudioParameters>(other);
        const auto this_sound_quality = sampleRate() * channels();
        const auto other_sound_quality = other_aparams->sampleRate() * other_aparams->channels();
